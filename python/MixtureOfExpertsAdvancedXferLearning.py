...
def undo_terminating_layers(network):
    sequential_modules = network._modules
    delete_keys = set()

    for key in reversed(sequential_modules.keys()):
        if type(sequential_modules[key]) is NNTools.ResidualBlock:
            break
        else:
            delete_keys.add(key)
            logging.info("Scheduling layer type %s for deletion" % 
                str(type(sequential_modules[key]))
            )

    for key in delete_keys:
        del sequential_modules[key]

    logging.info("Deleted relevant modules")

    for key in reversed(sequential_modules.keys()):
        logging.info(
            "Module type %s retained" % str(type(sequential_modules[key])))


def build_on_top(
    orig_model,
    read_convolver0_addendum=None,
    read_convolver1_addendum=None,
    compressor0_addendum=None,
    compressor1_addendum=None,
    compressor2_addendum=None,
    xattn0_addendum=None,
    xattn1_addendum=None,
    xattn2_addendum=None,
    combiner0_addendum=None,
    combiner1_addendum=None,
    meta_addendum=None,
):
    """
    Function to take an existing model and build more layers
    on top of each network component
    """
    # Model is wrapped in constructs for data parallel
    dnn = orig_model.module.dnn

    parameter_groups = {}
    parameter_group_counter = 0

    # Remove last layers of xattn block if we are upgrading the layer
    if hasattr(dnn, 'xattn0') and dnn.xattn0 is not None and xattn0_addendum is not None:
        undo_terminating_layers(dnn.xattn0.network)

    if hasattr(dnn, 'xattn1') and dnn.xattn1 is not None and xattn1_addendum is not None:
        undo_terminating_layers(dnn.xattn1.network)

    if hasattr(dnn, 'xattn2') and dnn.xattn2 is not None and xattn2_addendum is not None:
        undo_terminating_layers(dnn.xattn2.network)

    def attach_parameter_pair(item1_name, item2):
        nonlocal parameter_groups
        nonlocal parameter_group_counter

        if hasattr(dnn, item1_name):
            item1 = getattr(dnn, item1_name)
        else:
            return None
        ...
        if item1 is not None and item2 is not None:
            ...
            parameter_groups[
                "new_parameters%d" % parameter_group_counter
            ] = item2
    ...
    new_args["read_convolver1"] = attach_parameter_pair("read_convolver1", read_convolver1_addendum)
    new_args["compressor0"] = attach_parameter_pair("compressor0", compressor0_addendum)
    new_args["compressor1"] = attach_parameter_pair("compressor1", compressor1_addendum)
    new_args["compressor2"] = attach_parameter_pair("compressor2", compressor2_addendum)
    new_args["xattn0"] = attach_parameter_pair("xattn0", xattn0_addendum)
    new_args["xattn1"] = attach_parameter_pair("xattn1", xattn1_addendum)
    new_args["xattn2"] = attach_parameter_pair("xattn2", xattn2_addendum)
    new_args["combiner0"] = attach_parameter_pair("combiner0", combiner0_addendum)
    new_args["combiner1"] = attach_parameter_pair("combiner1", combiner1_addendum)

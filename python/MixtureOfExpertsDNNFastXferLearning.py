...
import MixtureOfExpertsAdvancedXferLearning as MixtureOfExpertsAdvanced
...
def train(
    numEpochs=10,
    lr_first_epoch_old=1e-8,
    lr_second_epoch_old=1e-4,
    lr_first_epoch_new=1e-3,
    lr_second_epoch_new=1e-4,
    configFile=None,
    cuda=True,
    ...
    # Convert config into dictionary of parameters
    def convert_to_params(dictionary):
        new_dictionary = {
            key: MixtureOfExpertsAdvanced.make_network(dictionary, key) for key in dictionary
        }
        return new_dictionary

    model = torch.load(model, map_location="cpu")
    configDict = convert_to_params(configDict)
    searcher, parameter_groups = MixtureOfExpertsAdvanced.build_on_top(model, **configDict)
    ...
    if onlyEval:
        raise NotImplementedError("Not supported for transfer learning")
    ...
    def create_optimizer(lr_old, lr_new):
        ...
        for param_key in parameter_groups:
            orig_key = "orig_parameters"
        ...
        optim = torch.optim.Adam(train_parameter_groups)

        return optim

    optim = create_optimizer(lr_first_epoch_old, lr_first_epoch_new)
    ...
    for module_item in parameter_groups.values():
        numParams += sum(p.numel() for p in module_item.parameters())
    ...
    for j in range(epochStart, numEpochs):
        ...
        if (scheduler is not None) and (optimizer != "SGDR"):
            raise NotImplementedError("Learning rate scheduler not allowed for transfer learning")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        help="Config for Mixture-of-Experts model extension",
        required=True,
    )
    ...
    parser.add_argument(
        "--lr_first_epoch_old",
        help="Learning rate for the first epoch for the old set of parameters",
        type=float,
        required=True,
    )
    ...
    parser.add_argument(
        "--lr_first_epoch_new",
        help="Learning rate for the first epoch for the new set of parameters",
        type=float,
        required=True,
    )
    ...
    parser.add_argument(
        "--lr_second_epoch_old",
        help="Learning rate for second and subsequent epochs for old set of parameters",
        type=float,
        required=True,
    )
    ...
    parser.add_argument(
        "--lr_second_epoch_new",
        help="Learning rate for second and subsequent epochs for new set of parameters",
        type=float,
        required=True,
    )
    ...
    parser.add_argument(
        "--model",
        help="Model path for transfer learning",
        required=True,
    )
    ...
    train(
        ...
        lr_first_epoch_new=args.lr_first_epoch_new,
        lr_second_epoch_new=args.lr_second_epoch_new,
        lr_first_epoch_old=args.lr_first_epoch_old,
        lr_second_epoch_old=args.lr_second_epoch_old,
